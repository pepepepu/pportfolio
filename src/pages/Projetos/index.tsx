...
import { ColorButton, NavigateButton, VideoGlitchTransition } from "../../components/molecules";
...
const Projetos: React.FC = () => {
  ...
  const buttons = [
    {
      text: "maMusic", onClick: () => {
        setShowGlitch(true);
        setTimeout(() => {
          navigate("projeto01");
        }, 400); // mesmo tempo que a animação de glitch
      }, background: "#FF0000", hover: "#ff9900"
    },
    { text: "BREEZE", onClick: () => null, background: "#00d0ff", hover: "#ff00ee" },
    { text: "TeamAPP", onClick: () => null, background: "#FFFF00", hover: "#a200ff" },
    { text: "fann_", onClick: () => null, background: "#00FF00", hover: "#0073ff" }
  ];
  ...
  return (
    ...
    >
      <Box width={"90%"} justifyContent={"space-between"} flexDirection={"row"}>
        <NavigateButton onClick={() => navigate("/home")} text={"HOME"} align={"flex-start"} />
      ...
      <Box width={"90%"} justifyContent={"space-between"} flexDirection={"row"}>
        <NavigateButton onClick={() => null} text={"QUEM SOU EU"} align={"flex-start"} />

        <Box width={"5%"}>
          <Text fontSize={"2rem"} color={"#FFF"} textShadow>+</Text>
        </Box>

        <NavigateButton onClick={() => null} text={"CONTATO"} align={"flex-end"} />
      </Box>
